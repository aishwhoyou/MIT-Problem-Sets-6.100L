...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    for i in freq_dict1:
        if i in freq_dict2:
            delta += abs(freq_dict1[i] - freq_dict2[i])
            sigma += freq_dict1[i] + freq_dict2[i]
        else:
            delta += freq_dict1[i]
            sigma += freq_dict1[i]
    for j in freq_dict2:
        if j not in freq_dict1:
            delta += freq_dict2[j]
            sigma += freq_dict2[j]
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    """
    max = 0
    combined_dict = {i: freq_dict1.get(i) + freq_dict2.get(i) for i in set(freq_dict1).union(freq_dict2)}
    for j in combined_dict:
        if combined_dict[j] > max:
            max = combined_dict[j]
    new_dict = {k: v for k, v in combined_dict.items() if v == max}
    freq_list = list(new_dict.keys())
    freq_list.sort()
    return freq_list
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    """
    word_dict = get_frequencies(text_to_list(load_file(file_path)))
    TF_dict = {key: value/(len(text_to_list(load_file(file_path)))) for (key, value) in word_dict.items()}
    return TF_dict
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for i in range(len(file_paths)):
        for j in get_frequencies(text_to_list(load_file(file_paths[i]))):
            if j in idf_dict:
                idf_dict[j] += 1
            else:
                idf_dict[j] = 1
    IDF_dict = {k: math.log(len(file_paths)/v ,10) for (k, v) in idf_dict.items()}
    return IDF_dict
...
def get_tfidf(tf_file_path, idf_file_paths):
    """
        ...
        * TF-IDF(i) = TF(i) * IDF(i)
        """
    tfidf_dict = {k: get_idf(idf_file_paths)[k]*get_tf(tf_file_path)[k] for k in get_tf(tf_file_path)}
    sol = tfidf_dict.items()
    return sorted(sol, key=lambda item: item[1])
